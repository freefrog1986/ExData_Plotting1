# This script is to  constract plot4.png in the repo
library(dplyr)
rm(list = ls())
...
par(mfrow = c(2,2))
with(sub_data, plot(sub_data$datatime,sub_data$Global_active_power,type = "n",ylab = "Global Active Power(kilowatts)",xlab = ""))
lines(sub_data$datatime,sub_data$Global_active_power)
with(sub_data, plot(sub_data$datatime,sub_data$Voltage,type = "n",ylab = "Voltage",xlab = "datetime"))
lines(sub_data$datatime,sub_data$Voltage)
...
with(sub_data, plot(sub_data$datatime,sub_data$Global_reactive_power,type = "n",ylab = "Global_reactive_power",xlab = "datetime"))
lines(sub_data$datatime,sub_data$Global_reactive_power)
dev.off()
